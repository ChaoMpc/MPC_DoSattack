...
%DoS attack sequence generation
attack=pars.attack;
actuatortimeFlag=1;
...
%Control loop
    if (i>1)
       Utemp.utemp=utemp;
       result=rigidNMPC(x(:, i), Utemp, pars);
       %print status.
       fprintf('%d: %s\n', i, result.status);
           if ~isequal(result.status, 'Solve_Succeeded')
              warning ('Failed at time %d!', i);
              break
          end
       utemp=result.u;
   else
       InitialNMPC.fixvar('x', 1, x0);
       InitialNMPC.solve();
       %print status.
...
if attack(:,i)<1
  ...
  actuatortimeFlag=actuatortimeFlag+1;
end
u(:,i)=actuator(:,actuatortimeFlag);
